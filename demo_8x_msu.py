...
# Might not do 8x, so ignore initially
...
cwd = os.getcwd()
# Not sure if we will even do 8x
path = os.path.join(cwd, 'checkpoints/IFRNet/IFRNet_GoPro.pth')
...
img8_path = os.path.join(cwd, 'figures/img1.png') 
img8_np = read(img8_path)
...
time_minference(img0_, img8_, embt)
...
gif_path = os.path.join(cwd, 'figures/out_8x.gif')
mimsave(gif_path, images, fps=9)
