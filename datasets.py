...
cwd = os.getcwd()
# vimeo_path = os.path.join(cwd, 'Datasets/Vimeo90K/vimeo_triplet')
vimeo_path = os.path.join(cwd, "Datasets", "Vimeo90K", "vimeo_triplet")
# msu_path = os.path.join(cwd, 'Datasets/MSU_Dataset/msu_triplet')
msu_path = os.path.join(cwd, "Datasets", "MSU_Dataset", "msu_triplet")

# gopro_path = os.path.join(cwd, 'Datasets/GOPRO')
gopro_path = os.path.join(cwd, "Datasets", "GOPRO")
...
class Vimeo90K_Train_Dataset(Dataset):
    def __init__(self, dataset_dir=vimeo_path, augment=True):
...
class Vimeo90K_Test_Dataset(Dataset):
    def __init__(self, dataset_dir=vimeo_path):
        self.dataset_dir = dataset_dir
        self.img0_list = []
        self.imgt_list = []
        self.img1_list = []
...
class MSU_Train_Dataset(Dataset):
    def __init__(self, dataset_dir=msu_path, augment=True):
        self.dataset_dir = dataset_dir
        self.augment = augment
        self.img0_list = []
        self.imgt_list = []
        ...
        self.flow_t0_list = []
        self.flow_t1_list = []
        with open(os.path.join(dataset_dir, 'tri_trainlist_raj.txt'), 'r') as f:
            for i in f:
                name = str(i).strip()
                if(len(name) <= 1):
                    continue
...
class MSU_Test_Dataset(Dataset):
    def __init__(self, dataset_dir=msu_path):
        self.dataset_dir = dataset_dir
        self.img0_list = []
        self.imgt_list = []
        self.img1_list = []
        ...
        if pretrained:
            tri_testlist = 'tri_testlist.txt'
        else:
            tri_testlist = 'tri_testlist_raj.txt'
            
        with open(os.path.join(dataset_dir, tri_testlist), 'r') as f:
            for i in f:
                name = str(i).strip()
                if(len(name) <= 1):
                    continue
...
class GoPro_Train_Dataset(Dataset):
    def __init__(self, dataset_dir=gopro_path, interFrames=7, n_inputs=2, augment=True):
        self.dataset_dir = dataset_dir
...
class GoPro_Test_Dataset(Dataset):
    def __init__(self, dataset_dir=gopro_path, interFrames=7, n_inputs=2):
        self.dataset_dir = dataset_dir
        self.interFrames = interFrames
        self.n_inputs = n_inputs
        self.setLength = (n_inputs-1)*(interFrames+1)+1
